...
# Hard Negative Mining (HNM): han che so luong cua negative Default Box = 3.Positive Box 
# Loss in regression task: MSE -> F.SmoothL1Loss (Huber Loss)
# Loss in classification task (multi classes): F.CrossEntropyLoss (Softmax Loss)
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        pos_idx = pos_mask.unsqueeze(2).expand_as(loc_data)  # (batch_size, num_dbox, 4)

        # positive boxes, loc_data
        ...
        loss_conf = loss_conf.view(num_batch, -1)      # torch.Size([num_batch, 8732])
            # Sap xep lai cac phan tu trong loss_conf theo thu tu giam dan
        _, loss_idx = loss_conf.sort(1, descending=True)  # (batch_size, num_dbox)
        ...
        pos_idx_mask = pos_mask.unsqueeze(2).expand_as(conf_data)  # (batch_size, num_dbox, num_classes)
